...
import { IAttributeType } from '../../config/types';
...
class DbTables extends cdk.Construct {
  ...
  constructor(scope: cdk.Construct, id: string, { stageName }: DbTablesProps) {
    ...
    for (const [key, tableSchema] of Object.entries(config.DB_SCHEMA)) {
    ...
    new dynamodb.Table(this, utils.getConstructId(tableName, stageName), {
      partitionKey: { name: 'id', type: dynamodb.AttributeType.STRING },
